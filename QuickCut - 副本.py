...
class CapsWriterTab(QWidget):
    ...
    def initGui(self):
        ...
        self.introBox.setPlainText(
            "选择阿里云 api 的引擎，启用 CapsWriter 语音输入后，只要在任意界面长按大写大写锁定键（Caps Lk）超过 0.3 秒，就会开始进行语音识别，说几句话，再松开大写锁定键，请别结果就会自动输入。你可以在这个输入框试试效果")
        self.masterLayout.addSpacing(30)
    ...
    def createDB(self):
        ...
        result = cursor.execute(
            'select * from %s where item = "%s";' % (preferenceTableName, 'CapsWriterTokenExpireTime'))
        if result.fetchone() == None:
            cursor.execute('''insert into %s (item, value) values ('%s', '%s');''' % (
    ...
    def capsWriterEnabled(self):
        ...
        cursor = conn.cursor()
        result = cursor.execute(
            '''update %s set value = 'True'  where item = '%s';''' % (preferenceTableName, 'CapsWriterEnabled'))
        conn.commit()
        api = cursor.execute('''select appkey, accessKeyId, accessKeySecret from %s where name = "%s"''' % (
        apiTableName, self.subtitleEngineComboBox.currentText())).fetchone()
        # 不在这里关数据库了()
    ...
    def capsWriterDisabled(self):
        ...
        cursor = conn.cursor()
        result = cursor.execute(
            '''update  %s set value = 'False'  where item = '%s';''' % (preferenceTableName, 'CapsWriterEnabled'))
        conn.commit()
        # 不在这里关数据库了()
...
class HelpTab(QWidget):
    def __init__(self):
        ...
        self.openVideoHelpButtone.clicked.connect(
            lambda: webbrowser.open(r'https://www.bilibili.com/video/BV18T4y1E7FF/'))
    ...
    def run(self):
        ...
        try:
            ...
            if self.whetherToUseOnlineSubtitleKeywordAutoCut:
                ...
                apiProvider, apiappKey, apiLanguage, apiAccessKeyId, apiAccessKeySecret = apiData[0], apiData[1], \
                                                                                          apiData[
...
class AutoSrtThread(QThread):
    ...
    def run(self):
        ...
        apiProvider, apiappKey, apiLanguage, apiAccessKeyId, apiAccessKeySecret = apiData[0], apiData[1], apiData[
            2], apiData[3], apiData[4]
        print('name: %s    provider: %s     appKey: %s    accessKeyId: %s   accessKeySecret: %s' % (
        self.apiEngine, apiProvider, apiappKey, apiAccessKeyId, apiAccessKeySecret))
        if apiProvider == 'Alibaba':
            transEngine = AliTrans()
        elif apiProvider == 'Tencent':
            transEngine = TencentTrans()
...
class CapsWriterThread(QThread):
    ...
    class MyCallback(SpeechRecognizerCallback):
        ...
        def on_result_changed(self, message):
            self.outputBox.print(
                '任务信息: task_id: %s, result: %s' % (message['header']['task_id'], message['payload']['result']))
    ...
    def get_token(self):
        newConn = sqlite3.connect(dbname)
        token = newConn.cursor().execute(
            'select value from %s where item = "%s";' % (preferenceTableName, 'CapsWriterTokenId')).fetchone()[0]
        expireTime = newConn.cursor().execute(
            'select value from %s where item = "%s";' % (preferenceTableName, 'CapsWriterTokenExpireTime')).fetchone()[
            0]
